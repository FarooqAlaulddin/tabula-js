...
- [ ] The sole publishable package `name` exactly matches P0-001.
- [ ] The React wrapper package and wrapper import aliases are absent from the v1 workspace.
- [ ] `rg` finds no old install command or package import specifier outside historical decision text.
- [ ] `pnpm install --frozen-lockfile`, build, typecheck, lint, unit tests, and e2e all pass.
- [ ] The Excalidraw example uses the core API directly and packed README examples use only the new name.
- [ ] The lockfile and all path aliases resolve without an unpublished placeholder package.
